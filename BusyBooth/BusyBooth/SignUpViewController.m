...
-(NSString*)sha256HashForFirstName:(NSString*) fName
    ...
    const char* str = [[NSString stringWithFormat:@"%@%@%@%d", fName, lName, DOB,
                        [[[address componentsSeparatedByString:@" "] firstObject] intValue]] UTF8String];
                                                ...
                                                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
                                      {
                                          ...
                                          if([[loginSuccessful objectForKey:@"code"] integerValue] == 1) {
                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                  [SVProgressHUD showErrorWithStatus:@"Signup Failed. Please check that you have entered your information correctly."];
                                              });
                                              ...
                                              [[NSUserDefaults standardUserDefaults] setObject:self.firstNameField.text forKey:@"fname"];
                                              [[NSUserDefaults standardUserDefaults] setObject:self.lastNameField.text forKey:@"lname"];
