...
// Store the timer that fires after a certain time
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [startButton addTarget:self
                  action:@selector(onStartPressed:)
  ...
  [stopButton setTitle:@"Checkout of Polling Place"
              forState:UIControlStateNormal];
  [stopButton setCenter:CGPointMake(width / 2, height * 6.6 / 7)];
  [stopButton addTarget:self
                 action:@selector(onStopPressed:)
       forControlEvents:UIControlEventTouchUpInside];
  ...
  //[[NSUserDefaults standardUserDefaults] objectForKey:@"isAdmin"];
  bool isAdmin = YES;
  if (isAdmin) {
    UIBarButtonItem *addTimeButton =
...
- (void)onAddPressed:(id)sender {
  UIAlertView *addTime =
    [[UIAlertView alloc] initWithTitle:@"Add wait time"
                               message:@"How many minutes is the current wait time?"
                              delegate:self
                     cancelButtonTitle:@"Cancel"
                     otherButtonTitles:@"This many minutes", nil];
...
- (void)alertView:(UIAlertView *)alertView
  ...
  if (buttonIndex == 1) {
    ...
    NSString *post =
      [NSString stringWithFormat:@"phone=%@&time=%@",
           [[NSUserDefaults standardUserDefaults] objectForKey:@"curr-number"], _addTimeField.text];
    NSData *postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
    NSString *postLength = [NSString stringWithFormat:@"%lu", (unsigned long)[postData length]];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init];

    // TODO: add /add_time route
    [request setURL:[NSURL URLWithString: @"http://localhost:5000/add_time"]];
